...
###################################################################################
    ...
    if verbose:
        print('\nNumber of Scales = {0:=03d}'.format(NScal))
...
###################################################################################
    ...
    cjk[:,0] = sig
    wps = np.zeros((NScal-1),dtype=float)
    ts = np.zeros(NScal-1,dtype=float)
    ...
    for dyada in np.arange(0,NScal-1):
        wps[dyada] = (djk[dyada]**2).sum()/sig_len
        tr = 2 * tr
        ts[dyada] = tr

    return cjk,djk,ts,wps
